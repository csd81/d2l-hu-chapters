...
Ebben a szakaszban bemutatjuk a *gradient descent* mögött álló alapfogalmakat.
Bár a mélytanulásban ritkán alkalmazzák közvetlenül, a gradient descent megértése kulcsfontosságú a sztochasztikus gradient descent algoritmusok megértéséhez.
Például az optimalizálási probléma eltérhet a megoldástól a túlságosan nagy tanulási sebesség miatt. Ez a jelenség már a gradient descent esetén is megfigyelhető. Hasonlóképpen, az előkondicionálás a gradient descent egyik általánosan alkalmazott technikája, amely a fejlettebb algoritmusokra is érvényes.
Kezdjük egy egyszerű speciális esettel.


## Egydimenziós gradient descent

Az egydimenziós gradient descent kiváló példa annak magyarázatára, hogy a gradient descent algoritmus miért csökkentheti a célfüggvény értékét. Tekintsünk egy folytonosan differenciálható, valós értékű $f: \mathbb{R} \rightarrow \mathbb{R}$ függvényt. Taylor-kifejtéssel kapjuk:
...
frissítési szabályt alkalmazzuk $x$ iterálásához, az $f(x)$ értéke csökkenhet. Ezért a gradient descent során először egy kezdeti $x$ értéket és egy $\eta > 0$ konstanst választunk, majd ezekkel folyamatosan iteráljuk $x$-et, amíg a leállítási feltétel nem teljesül – például amikor a gradiens nagysága $|f'(x)|$ elég kicsi, vagy az iterációk száma elér egy bizonyos értéket.

Az egyszerűség kedvéért az $f(x)=x^2$ célfüggvényt választjuk a gradient descent megvalósításának szemléltetéséhez. Bár tudjuk, hogy $x=0$ a megoldás $f(x)$ minimalizálásához, ezt az egyszerű függvényt mégis használjuk annak megfigyeléséhez, hogyan változik $x$.
...
Ezután $x=10$-et használjuk kezdőértékként, és $\eta=0.2$-t feltételezünk. A gradient descent segítségével 10-szer iterálva $x$-et, láthatjuk, hogy $x$ értéke végül közelíti az optimális megoldást.
...
Annak megértéséhez, hogy ez miért lehet jó ötlet, képzeljük el, hogy az egyik változó milliméteres magasságot, a másik kilométeres magasságot jelöl. Feltételezve, hogy mindkettőnél a természetes skála méterben van, szörnyű eltérés van a parameterizációban. Szerencsére az előkondicionálás alkalmazása megszünteti ezt. A gradient descenttel kombinált előkondicionálás lényegében különböző tanulási sebességet jelent az egyes változók (a $\mathbf{x}$ vektor koordinátái) számára.
Ahogy később látni fogjuk, az előkondicionálás számos innováció motorja a sztochasztikus gradient descent optimalizálási algoritmusokban.


### Gradient descent vonalkereséssel

A gradient descent egyik kulcsproblémája, hogy túllőhetünk a célon, vagy nem haladunk elég gyorsan. Egyszerű megoldás a vonalkeresés kombinálása a gradient descenttel. Vagyis a $\nabla f(\mathbf{x})$ által adott irányt használjuk, majd bináris keresést végzünk annak meghatározására, hogy melyik $\eta$ tanulási sebesség minimalizálja az $f(\mathbf{x} - \eta \nabla f(\mathbf{x}))$ értéket.
...
* A gradient descent beragadhat lokális minimumokba.
...
1. Kísérletezz különböző tanulási sebességekkel és célfüggvényekkel gradient descent esetén.
...
1. Tervezz egy $\mathbb{R}^2$-en definiált célfüggvényt, amelynél a gradient descent rendkívül lassú. Tipp: skálázd a különböző koordinátákat eltérően.
