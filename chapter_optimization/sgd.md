# Sztochasztikus gradient descent
:label:`sec_sgd`

A korábbi fejezetekben folyamatosan sztochasztikus gradient descentet alkalmaztunk a tanítási eljárásunkban, anélkül azonban, hogy megmagyaráztuk volna, miért működik.
Ennek megvilágítása érdekében
a :numref:`sec_gd` szakaszban éppen bemutattuk a gradient descent alapelveit.
Ebben a szakaszban a *sztochasztikus gradient descent* részletesebb tárgyalásával folytatjuk.
...
Ha gradient descentet alkalmazunk, minden egyes független változó iteráció számítási költsége $\mathcal{O}(n)$, ami $n$-nel lineárisan nő. Ezért minél nagyobb a tanítóhalmaz, annál nagyobb a gradient descent iterációnkénti költsége.

A sztochasztikus gradient descent (SGD) csökkenti az iterációnkénti számítási költséget. A sztochasztikus gradient descent minden egyes iterációjában egyenletesen véletlenszerűen mintavételezünk egy $i\in\{1,\ldots, n\}$ indexet az adatpéldákhoz, és a $\nabla f_i(\mathbf{x})$ gradiensszámítással frissítjük $\mathbf{x}$-et:

$$\mathbf{x} \leftarrow \mathbf{x} - \eta \nabla f_i(\mathbf{x}),$$

ahol $\eta$ a tanulási sebesség. Látható, hogy az iterációnkénti számítási költség csökken a gradient descent $\mathcal{O}(n)$-jéről a konstans $\mathcal{O}(1)$-re. Ráadásul ki kell emelnünk, hogy a sztochasztikus gradiens $\nabla f_i(\mathbf{x})$ a teljes gradiens $\nabla f(\mathbf{x})$ torzítatlan becslése, mivel:

$$\mathbb{E}_i \nabla f_i(\mathbf{x}) = \frac{1}{n} \sum_{i = 1}^n \nabla f_i(\mathbf{x}) = \nabla f(\mathbf{x}).$$

Ez azt jelenti, hogy átlagosan a sztochasztikus gradiens jó közelítése a valódi gradiensnek.

Most összehasonlítjuk a gradient descenttel úgy, hogy 0 várható értékű és 1 varianciájú véletlenszerű zajt adunk a gradienhez, szimulálva a sztochasztikus gradient descentet.
...
Ahogy látható, a sztochasztikus gradient descentben a változók trajektóriája sokkal zajosabb, mint a :numref:`sec_gd` szakaszban megfigyelt gradient descent esetén. Ez a gradiens sztochasztikus természetéből adódik. Vagyis még akkor is, amikor a minimum közelébe érünk, az $\eta \nabla f_i(\mathbf{x})$ pillanatnyi gradiens által bevezetett bizonytalanság továbbra is fennáll. Még 50 lépés után is meglehetősen gyenge a minőség. Sőt, további lépésekkel sem javul (bátorítunk arra, hogy több lépéssel is kísérletezz ennek megerősítéséhez). Ez csak egy alternatívát hagy: a $\eta$ tanulási sebesség megváltoztatását. Ha azonban túl kicsit választunk, kezdetben nem érünk el érdemi haladást. Másrészt, ha túl nagyot választunk, nem kapunk jó megoldást, ahogy fentebb is láttuk. E két ellentétes cél feloldásának egyetlen módja a tanulási sebesség *dinamikus* csökkentése az optimalizálás előrehaladtával.

Ez az oka annak is, hogy egy `lr` tanulási sebesség függvényt adtunk hozzá az `sgd` lépésfüggvényhez. A fenti példában a tanulási sebesség ütemező funkcionalitása inaktív, mivel az ahhoz tartozó `lr` függvényt állandóként állítottuk be.

## Dinamikus tanulási sebesség

Az $\eta$ helyére egy időfüggő $\eta(t)$ tanulási sebesség behelyettesítése növeli az optimalizálási algoritmus konvergenciájának szabályozásának összetettségét. Különösen azt kell kitalálni, hogy $\eta$-nak milyen gyorsan kell csökkenie. Ha túl gyorsan csökken, idő előtt befejezzük az optimalizálást. Ha túl lassan csökkentjük, túl sok időt pazarlunk az optimalizálásra. Az alábbiak néhány alapvető stratégia, amelyet az $\eta$ időbeli beállításához alkalmaznak (a fejlettebb stratégiákat később tárgyaljuk):
...
Számos más lehetőség is létezik a tanulási sebesség beállítására. Például kezdhetünk kis sebességgel, majd gyorsan növelhetjük, majd ismét csökkenthetjük, bár lassabban. Akár váltakozhatunk kisebb és nagyobb tanulási sebességek között is. Sokféle ilyen ütemterv létezik. Egyelőre összpontosítsunk azokra a tanulási sebesség ütemezőkre, amelyekre átfogó elméleti elemzés lehetséges, vagyis a konvex esetben alkalmazott tanulási sebességekre. Általános nemkonvex problémáknál nagyon nehéz érdemleges konvergenciagaranciákat kapni, mivel általánosan nemlineáris nemkonvex problémák minimalizálása NP-nehéz. Áttekintésért lásd például Tibshirani 2015-ös kiváló [előadásjegyzeteit](https://www.stat.cmu.edu/%7Eryantibs/convexopt-F15/lectures/26-nonconvex.pdf).



## Konvergenciaanalízis konvex célfüggvényekre

A sztochasztikus gradient descent következő konvergenciaanalízise konvex célfüggvényekre opcionális, és elsősorban a probléma jobb megértéséhez szükséges intuíció átadását szolgálja.
...
Konkrétabban
a sztochasztikus gradient descent frissítését tekintjük:
...
Ez azt jelenti, hogy addig haladunk, amíg az aktuális veszteség és az optimális veszteség különbsége meghaladja $\eta_t L^2/2$-t. Mivel ez a különbség szükségszerűen nullához tart, következik, hogy a $\eta_t$ tanulási sebességnek szintén el kell *tűnnie*.
...
* Konvex problémáknál bebizonyítható, hogy a tanulási sebességek széles körű megválasztása esetén a sztochasztikus gradient descent konvergál az optimális megoldáshoz.
* A mélytanulásban ez általában nem igaz. A konvex problémák elemzése azonban hasznos betekintést nyújt az optimalizálás megközelítéséhez: a tanulási sebességet fokozatosan, de nem túl gyorsan kell csökkenteni.
* Problémák lépnek fel, ha a tanulási sebesség túl kicsi vagy túl nagy. A gyakorlatban a megfelelő tanulási sebességet általában csak több kísérlet után találják meg.
* Ha a tanítóhalmazban több példa van, minden gradient descent iteráció kiszámítása drágább, ezért ezekben az esetekben a sztochasztikus gradient descent az előnyösebb.
...
1. Kísérletezz különböző tanulási sebesség ütemezőkkel a sztochasztikus gradient descenthez, és különböző számú iterációval. Különösen ábrázold az optimális megoldástól való $(0, 0)$ távolságot az iterációk számának függvényében.
...
1. Hasonlítsd össze a sztochasztikus gradient descent konvergenciáját, ha visszatevéssel és visszatevés nélkül mintavételez az $\{(x_1, y_1), \ldots, (x_n, y_n)\}$ halmazból.
1. Hogyan módosítanád a sztochasztikus gradient descent megoldót, ha egyes gradiensek (pontosabban a hozzájuk tartozó koordináták) következetesen nagyobbak lennének az összes többi gradiensnél?
