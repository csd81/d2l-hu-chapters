...
ahol $\eta$ a tanulási sebesség. Látható, hogy az iterációnkénti számítási költség csökken a gradienscsökkenés $\mathcal{O}(n)$-jéről a konstans $\mathcal{O}(1)$-re. Ráadásul ki kell emelnünk, hogy a sztochasztikus gradiens $\nabla f_i(\mathbf{x})$ a teljes gradiens $\nabla f(\mathbf{x})$ torzítatlan becslése, mivel:
...
* Konvex problémáknál bebizonyítható, hogy a tanulási sebességek széles körű megválasztása esetén a sztochasztikus gradienscsökkenés konvergál az optimális megoldáshoz.
...
1. Kísérletezz különböző tanulási sebesség ütemezőkkel a sztochasztikus gradienscsökkenéshez, és különböző számú iterációval. Különösen ábrázold az optimális megoldástól való $(0, 0)$ távolságot az iterációk számának függvényében.
