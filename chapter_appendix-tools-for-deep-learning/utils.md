...
Ez a szakasz tartalmazza a könyvben használt segédfüggvények és osztályok megvalósításait.
...
def save_hyperparameters(self, ignore=[]):
    """A függvény argumentumainak mentése osztályattribútumokba."""
...
FrozenLake környezet hozzáadása
```{.python .input}
%%tab pytorch

def frozen_lake(seed): #@save
    # A környezetről itt találsz többet: https://www.gymlibrary.dev/environments/toy_text/frozen_lake/
    # Az env.P.items feldolgozása innen lett átdolgozva: https://sites.google.com/view/deep-rl-bootcamp/labs
    import gym
    ...
    env_info = {}
    env_info['desc'] = env.desc  # 2D tömb, amely megadja, mit jelent minden rácselem
    env_info['num_states'] = env.nS  # Megfigyelések/állapotok száma
    env_info['num_actions'] = env.nA  # Akciók száma
    # Indexek definiálása a (transition probability, nextstate, reward, done) tuple-höz
    env_info['trans_prob_idx'] = 0  # Az átmenetivalószínűség-bejegyzés indexe
    env_info['nextstate_idx'] = 1  # A következő állapot bejegyzésének indexe
    env_info['reward_idx'] = 2  # A jutalom bejegyzésének indexe
    env_info['done_idx'] = 3  # A befejezettséget jelző bejegyzés indexe
    env_info['mdp'] = {}
    env_info['env'] = env
...
def make_env(name ='', seed=0): #@save
    ...
    # name: egy gym környezet neve.
    # Értékiterációhoz csak a FrozenLake-v1 támogatott.
...
Értékfüggvény megjelenítése
```{.python .input}
%%tab pytorch

def show_value_function_progress(env_desc, V, pi): #@save
    # Ez a függvény azt jeleníti meg, hogyan változik az értékfüggvény és a politika az időben.
    ...
    for k in range(V.shape[0]):
        ...
        for y in range(4):
            for x in range(4):
                ...
                # A G és H címkéjű cellákhoz nem rajzolunk nyilat
                if env_desc[y,x].decode() != 'G' and env_desc[y,x].decode() != 'H':
                    ax.arrow(x, y, dx, dy, color='r', head_width=0.2, head_length=0.15)

        ax.set_title("Lépés = "  + str(k + 1), fontsize=20)

    fig.tight_layout()
    plt.show()

```
Q-függvény megjelenítése
```{.python .input}
%%tab pytorch
...
def show_Q_function_progress(env_desc, V_all, pi_all): #@save
    ...
    for k in range(V.shape[0]):
        ...
        ax.set_title("Lépés = "  + str(vis_indx[k] + 1), fontsize=20)
